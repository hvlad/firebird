...
#endif
	BulkInsert* bulkInsert = nullptr;
	if (bulk)
	{
		blob->blb_flags |= BLB_bulk;
		bulkInsert = transaction->getBulkInsert(tdbb, relation);
	}

	if (bulkInsert)
		destination->set_permanent(relation->rel_id, bulkInsert->putBlob(tdbb, blob, record));
	else
